...
const UserPanel = ({ onClose }) => {
  ...
  const handleDelete = (username) => {
    if ((['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(username))) {
      window.$alert('Usuários mestres (danilo, ryan.santos, carol.cons, talita.alves) não podem ser excluídos.');
      return;
    }
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input 
                  ...
                  disabled={editingUser && (['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(formData.username))}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', background: '#2a2a2a', padding: '1rem', borderRadius: '6px', border: '1px solid #444' }}>
                ...
                <label style={{ color: '#fff', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                  <input type="checkbox" checked={formData.permissions?.includes('db') || (['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(formData.username)) || formData.role === 'superadmin'} onChange={(e) => {
                ...
                <label style={{ color: '#fff', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                  <input type="checkbox" checked={formData.permissions?.includes('contabil') || (['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(formData.username)) || formData.role === 'superadmin'} onChange={(e) => {
                ...
                <label style={{ color: '#fff', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                  <input type="checkbox" checked={formData.permissions?.includes('dash') || (['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(formData.username)) || formData.role === 'superadmin'} onChange={(e) => {
                ...
                <label style={{ color: '#fff', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                  <input type="checkbox" checked={formData.permissions?.includes('estoque') || (['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(formData.username)) || formData.role === 'superadmin'} onChange={(e) => {
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
                  <div style={{ flex: 1, paddingRight: '0.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                      <strong style={{ color: (['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(u.username) || u.role === 'superadmin') ? 'var(--color-primary)' : '#fff', wordBreak: 'break-all' }}>{u.username}</strong>
                    ...
                    <div style={{ marginTop: '0.4rem' }}>
                      {(['danilo', 'ryan.santos', 'carol.cons', 'talita.alves'].includes(u.username) || u.role === 'superadmin') ? (
